...
串行 CSR SpMV 的输出向量在每一行都会被完整赋值，原实现的 `setZero()` 只增加一次
全向量写入。去掉这次清零后，用同一个 102400 单元 cavity、IC/ILUT、20 次外迭代和
相同的 18636 次 Krylov 迭代做三次正式 1-rank A/B：基线 solver 为 21.81--23.25 s
（中位数 21.81 s），当前路径为 20.68--21.35 s（中位数 21.00 s）；SpMV 分别为
4.89--6.18 s（中位数 5.40 s）和 4.47--4.80 s（中位数 4.58 s）。墙钟中位数为
24.94 s 与 24.09 s。两条路径的线性求解次数、Krylov 迭代数、稀疏乘次数和返回状态
一致；这是固定吞吐 pilot，约 3.7% 的 solver 中位数差异和约 15% 的 SpMV 中位数差异
受运行噪声影响，不能视为跨平台的固定收益。

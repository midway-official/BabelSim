...
double difference(double a, double b) { require(std::isfinite(a) && std::isfinite(b), "nonfinite scalar"); return magnitude(a-b); }
double difference(Vec3 a, Vec3 b) { require(std::isfinite(norm(a)) && std::isfinite(norm(b)), "nonfinite vector"); return magnitude(a-b); }
...
struct Fields {
    explicit Fields(const Mesh& mesh)
        ...
        for (Index patch = 0; patch < mesh.patchCount(); ++patch) {
            ...
            p.boundary(patch) = zeroGradient();
            k.boundary(patch) = zeroGradient();
            U.boundary(patch) = zeroGradient();
        }
    }
    ScalarField p, k;
    VectorField U, gradP;
...
struct Answers {
    ...
    void check(const ScalarField& value, bool record) {
        if (record) scalars.push_back(value); else { double e=compare(value,scalars.at(si++)); if(e>1e-10) std::cout<<"scalarCheck="<<si<<" error="<<e<<std::endl; error=std::max(error,e); }
    }
    void check(const VectorField& value, bool record) {
        if (record) vectors.push_back(value); else { double e=compare(value,vectors.at(vi++)); if(e>1e-10) std::cout<<"vectorCheck="<<vi<<" error="<<e<<std::endl; error=std::max(error,e); }
    }
...
int main(int argc, char* argv[]) {
    ...
    for (DiffusionMethod method : {DiffusionMethod::Orthogonal, DiffusionMethod::Corrected,
        ...
        control.methods.gradient = GradientMethod::LeastSquares;
    ...
    try {
        ...
        std::cout << "REVIEW GG maximum=" << maximum << std::endl;

...
## 2026-09-16 过程式模型重构验证

当前实现已改为三个独立模型文件，使用 `solveTransport()` 显式组装和求解。
`model.cpp` 只负责选择及连接；下文 2026-09-09 记录中的 `simple_common.h`、
共享动量实现和临时字段名称属于历史结构，当前接口以
[RANS 开发说明](../../src/physics/RANS/README.md) 为准。

本轮查阅 TMR 标准 SA、Wilcox1988m 定义及 OpenFOAM 10 k-epsilon 实现。
模型版本、常数、两方程冻结系数顺序、下限及 SA 正 S-tilde 保护保持不变。
每条输运方程仅组装一次，在松弛前记录归一化残差。报告保留逐方程线性结果，
不合并不同物理变量的有量纲绝对残差；物理收敛和打印仍由 SIMPLE 负责。

`make test-rans` 自然退出 0。独立标量公式构造的单次输运参考解与实际解一致，
同时检查报告名称、松弛前残差、有效黏度和 SA 零壁面迹；不依赖模型私有临时场。
全套稳态/瞬态 1/2/4 进程比较与裁剪拒绝检查通过。

| 模型 | Euler 相邻加密误差比 | BDF2 相邻加密误差比 |
| --- | --- | --- |
| SA | 1.955 / 1.977 | 4.138 / 4.029 |
| kOmega | 1.998 / 1.996 | 3.962 / 3.785 |
| kEpsilon | 1.982 / 1.991 | 4.044 / 4.020 |

本地证据：`/tmp/babelsim-rans-validation-ct50sxby/summary.json`。
这些结果证明本套方程及时间离散测试通过，不代表自动壁函数或复杂工程壁流已验证。

...
实验使用 [benchmark-re1000](../../cases/cavity/benchmark-re1000) 案例：由规则外形离线生成的
`64×64×1` 显式六面体连接、双曲正切壁面加密 `cluster=1.5`、`Re=1000`、`linearUpwind` 对流、最小二乘梯度和中心
...
这份旧实现快照只说明当时的开销组成。当前后端不提供 GMRES；对大规模三维扩展，应在
更大局部问题上重新调节粗层阈值、平滑次数，并实现分布式粗层/进程聚合后，再与 ILUT
做强扩展和弱扩展对照。小网格历史结果不代表当前 AMG 在所有 CFD 问题上都更快。
